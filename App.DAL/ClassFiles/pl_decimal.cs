// <fileinfo name="pl_decimalCollection_Base.cs">
//		<copyright>
//			All rights reserved.
//		</copyright>
//		<remarks>
...
{
	/// <summary>
	/// The base class for <see cref="pl_decimalCollection"/>. Provides methods 
	...
	public  class pl_decimal
	{
		...
		/// <summary>
		/// Gets an array of all records from the <c>pl_decimal</c> table.
		/// </summary>
		/// <returns>An array of <see cref="pl_decimalRow"/> objects.</returns>
		public virtual pl_decimalRow[] GetAll()
		{
		...
		/// includes all records from the <c>pl_decimal</c> table.
		/// </summary>
		...
		/// to retrieve all records from the <c>pl_decimal</c> table.
		/// </summary>
		...
		{
			return _db.CreateCommand("dbo._pl_decimal_GetAll", true);
		}

		/// <summary>
		/// Gets the first <see cref="pl_decimalRow"/> objects that 
		/// match the search condition.
		/// </summary>
		/// <param name="whereSql">The SQL search condition. For example: 
		/// <c>"FirstName='Smith' AND Zip=75038"</c>.</param>
		/// <returns>An instance of <see cref="pl_decimalRow"/> or null reference 
		/// (Nothing in Visual Basic) if the object was not found.</returns>
		public pl_decimalRow GetRow(string whereSql)
		{
			int totalRecordCount = -1;
			pl_decimalRow[] rows = GetAsArray(whereSql, null, 0, 1, ref totalRecordCount);
			return 0 == rows.Length ? null : rows[0];
		}

		/// <summary>
		/// Gets an array of <see cref="pl_decimalRow"/> objects that 
		...
		/// <returns>An array of <see cref="pl_decimalRow"/> objects.</returns>
		public pl_decimalRow[] GetAsArray(string whereSql, string orderBySql)
		{
			int totalRecordCount = -1;
		...
		/// <returns>An array of <see cref="pl_decimalRow"/> objects.</returns>
		public virtual pl_decimalRow[] GetAsArray(string whereSql, string orderBySql,
		...
		{
			string sql = "SELECT * FROM [dbo].[pl_decimal]";
		...
		/// <summary>
		/// Adds a new record into the <c>pl_decimal</c> table.
		/// </summary>
		...
		/// <summary>
		/// Deletes <c>pl_decimal</c> records that match the specified criteria.
		/// </summary>
		...
		/// to delete <c>pl_decimal</c> records that match the specified criteria.
		/// </summary>
		/// <param name="whereSql">The SQL search condition. 
		/// For example: <c>"FirstName='Smith' AND Zip=75038"</c>.</param>
		/// <returns>A reference to the <see cref="System.Data.SqlCommand"/> object.</returns>
		...
		{
			string sql = "DELETE FROM [dbo].[pl_decimal]";
			if(null != whereSql && 0 < whereSql.Length)
				sql += " WHERE " + whereSql;
			return _db.CreateCommand(sql);
		}
		...
		/// <summary>
		/// Deletes all records from the <c>pl_decimal</c> table.
		/// </summary>
		/// <returns>The number of deleted records.</returns>
		...
		/// <returns>An array of <see cref="pl_decimalRow"/> objects.</returns>
		protected pl_decimalRow[] MapRecords(SqlCommand command)
		{
		...
		/// <returns>An array of <see cref="pl_decimalRow"/> objects.</returns>
		protected pl_decimalRow[] MapRecords(SqlDataReader reader)
		{
			int totalRecordCount = -1;
		...
		/// of records in the reader object if 0 was passed into the method; otherwise it returns -1.</param>
		/// <returns>An array of <see cref="pl_decimalRow"/> objects.</returns>
		protected virtual pl_decimalRow[] MapRecords(SqlDataReader reader, 
										int startIndex, int length, ref int totalRecordCount)
		{
			if(0 > startIndex)
			...
			while(reader.Read())
			{
				...
				if(ri > 0 && ri <= length)
				{
					pl_decimalRow record = new pl_decimalRow();
					...
					if(!reader.IsDBNull(data_valueColumnIndex))
						record.Data_value = Convert.ToDecimal(reader.GetValue(data_valueColumnIndex));
			...
			return (pl_decimalRow[])(recordList.ToArray(typeof(pl_decimalRow)));
		}
		...
		/// <summary>
		/// Converts <see cref="System.Data.DataRow"/> to <see cref="pl_decimalRow"/>.
		/// </summary>
		/// <param name="row">The <see cref="System.Data.DataRow"/> object to be mapped.</param>
		/// <returns>A reference to the <see cref="pl_decimalRow"/> object.</returns>
		protected virtual pl_decimalRow MapRow(DataRow row)
		{
			pl_decimalRow mappedObject = new pl_decimalRow();
			...
			if(!row.IsNull(dataColumn))
				mappedObject.Data_value = (decimal)row[dataColumn];
			return mappedObject;
		}

		/// <summary>
		/// Creates a <see cref="System.Data.DataTable"/> object for 
		/// the <c>pl_decimal</c> table.
		/// </summary>
		/// <returns>A reference to the <see cref="System.Data.DataTable"/> object.</returns>
		protected virtual DataTable CreateDataTable()
		{
			DataTable dataTable = new DataTable();
			dataTable.TableName = "pl_decimal";
			DataColumn dataColumn;
			...
			dataColumn = dataTable.Columns.Add("Data_value", typeof(decimal));
	...
	} // End of pl_decimalCollection_Base class
}  // End of namespace
